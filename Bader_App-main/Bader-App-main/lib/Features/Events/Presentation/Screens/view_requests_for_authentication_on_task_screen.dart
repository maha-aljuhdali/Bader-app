...
class ViewRequestsForAuthenticateOnTaskScreen extends StatelessWidget {
  ...
  const ViewRequestsForAuthenticateOnTaskScreen({Key? key, required this.task})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    final EventsCubit eventCubit = EventsCubit.getInstance(context)
      ..getRequestForAuthenticateOnATask(taskID: task.id.toString().trim());
    return Directionality(
      textDirection: TextDirection.rtl,
      child: SafeArea(
        child: Scaffold(
          appBar: AppBar(
            title: const Text("مصادقة المهام"),
          ),
          body: BlocConsumer<EventsCubit, EventsStates>(
              buildWhen: (pastState, currentState) =>
                  currentState
                      is GetRequestForAuthenticateOnATaskSuccessState ||
                  currentState is FailedToGetRequestForAuthenticateOnATaskState,
              listener: (context, state) {
                if (state
                    is AcceptOrRejectAuthenticateRequestOnATaskLoadingState) {
                  showLoadingDialog(context: context);
                }
                if (state
                    is AcceptOrRejectAuthenticateRequestOnATaskSuccessState) {
                  Navigator.pop(context);
                  showToastMessage(
                      context: context, message: "تم المصادقة بنجاح");
                }
              },
              builder: (context, state) {
                return Padding(
                  ...
                  child: state is GetRequestForAuthenticateOnATaskLoadingState
                      ? const Center(child: CircularProgressIndicator())
                      : state is GetRequestForAuthenticateOnATaskSuccessState &&
                              state.requests.isNotEmpty
                          ? ListView.separated(
                              physics: const BouncingScrollPhysics(),
                              itemCount: state.requests
                              ...
                              itemBuilder: (context, index) => _requestItem(
                                  ...
                                  context: context,
                                  requestData: state.requests[index],
                                  task: task))
                          : Center(
                              child: Text(
                                "لا توجد طلبات للمصادقة",
                                style: TextStyle(
                                    fontSize: 15.sp,
  ...
  Widget _requestItem(
      ...
      required RequestAuthenticationOnATaskModel requestData}) {
    return Card(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _textSpan(
                title: "إسم الفعالية",
                value:
                    "${task.forPublicOrSpecificToAnEvent ? "غير خاص بفعالية معينة" : task.eventName}"),
            ...
            Row(
              ...
              children: [
                _buttonItem(
                    ...
                    onTap: () {
                      eventsCubit.acceptOrRejectAuthenticateRequestOnATask(
                          myID: layoutCubit.userData!.id ?? Constants.userID!,
                          layoutCubit: layoutCubit,
                          requestSenderName: requestData.senderName,
                          requestFirebaseFCMToken:
                              requestData.senderFirebaseFCMToken,
                          taskEntity: task,
