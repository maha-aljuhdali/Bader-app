...
class ViewOpinionsAboutEventScreen extends StatelessWidget {
  ...
  const ViewOpinionsAboutEventScreen({Key? key, required this.eventID})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: SafeArea(
        child: Scaffold(
          appBar: AppBar(
            title: const Text("الأراء"),
          ),
          body: BlocConsumer<EventsCubit, EventsStates>(
              buildWhen: (pastState, currentState) =>
                  currentState is GetOpinionsAboutEventSuccessState ||
                  currentState is FailedToGetOpinionsAboutEventState,
              ...
              builder: (context, state) {
                return Padding(
                  ...
                  child: state is GetOpinionsAboutEventLoadingState
                      ? const Center(child: CircularProgressIndicator())
                      : state is GetOpinionsAboutEventSuccessState &&
                              state.opinions.isNotEmpty
                          ? ListView.separated(
                              physics: const BouncingScrollPhysics(),
                              itemCount: state.opinions
                              ...
                              itemBuilder: (context, index) => _opinionItem(
                                  index: index,
                                  context: context,
                                  opinionData: state.opinions[index]),
                            )
                          : Center(
                              child: Text(
                                "لا توجدأراء عن الفعالية",
                                style: TextStyle(
                                    fontSize: 15.sp,
  ...
  Widget _opinionItem(
      ...
      required int index,
      required OpinionAboutEventModel opinionData}) {
    return Card(
      child: Padding(
        padding: EdgeInsets.symmetric(vertical: 12.h, horizontal: 12.w),
        child: Text(
          "${index + 1} - ${opinionData.opinion!}",
