...
class MembershipRequestsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final cubit = ClubsCubit.getInstance(context)
      ..getMembershipRequests(clubID: clubID);
    String clubName = cubit.dataAboutClubYouLead!.name!;
    return Directionality(
      textDirection: TextDirection.rtl,
      child: Scaffold(
        appBar: AppBar(
          title: const Text("طلبات العضوية"),
        ),
        body: BlocConsumer<ClubsCubit, ClubsStates>(
          buildWhen: (last, currentState) =>
              currentState is GetMembershipRequestSuccessState,
          listener: (context, state) {
            if (state is AcceptOrRejectMembershipRequestSuccessState) {
              Navigator.pop(
                  context); // TODO: To get out from Alert Dialog ( showLoadingDialog )
                  ...
                  child: cubit.membershipRequests.isNotEmpty
                      ? ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            return _requestMembershipItem(
                                context: context,
                                clubName: clubName,
                                layoutCubit: layoutCubit,
                                cubit: cubit,
                                requestData: cubit.membershipRequests[index],
                                clubID: clubID);
                          },
                        )
...
Widget _requestMembershipItem(
    {required BuildContext context,
    required RequestMembershipEntity requestData,
    required LayoutCubit layoutCubit,
    required ClubsCubit cubit,
    required String clubID,
    required String clubName}) {
  return InkWell(
    onTap: () => showMemberData(
        context: context,
        committeeName: requestData.committeeName!,
        aboutMember: requestData.infoAboutSender!),
    child: Card(
      elevation: 0.2,
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                _buttonItem(
                    requestSenderFirebaseFCMToken:
                        requestData.senderFirebaseFCMToken,
                    committeeNameForRequestSender: requestData.committeeName,
                    clubID: clubID,
                    cubit: cubit,
                    responseStatus: true,
...
Widget _buttonItem(
    ...
    String? requestSenderFirebaseFCMToken,
    required String clubID,
    required String clubName,
    required LayoutCubit layoutCubit}) {
  return MaterialButton(
      ...
      height: 30.h,
      onPressed: () => cubit.acceptOrRejectMembershipRequest(
          receiverFirebaseMessagingToken: requestSenderFirebaseFCMToken,
          committeeNameForRequestSender: committeeNameForRequestSender,
          idForClubILead: clubID,
          requestSenderID: requestSenderID,
          clubID: clubID,
          respondStatus: responseStatus,
          layoutCubit: layoutCubit,
          clubName: clubName),
      color: responseStatus ? AppColors.kGreenColor : AppColors.kRedColor,
      child: Text(
        responseStatus ? 'قبول' : "رفض",
      ));
}
