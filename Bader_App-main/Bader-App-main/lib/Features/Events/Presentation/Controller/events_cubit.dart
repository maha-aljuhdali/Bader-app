...
class EventsCubit extends Cubit<EventsStates> {
  ...
  List<String> namesForEventsICreated =
      []; // TODO: Mean that created by me | Related Club That I lead  راح نستخدمها في حاله انشاء تاسك مع DropDownButton
  List<EventEntity> allEvents = [];
  // TODO: احنا مسوين idForClubILead مو required لأن راح نستخدمها في حاله اذا كان المستخدم هذا بالفعل قائد ف بيحتاج نشوفه عشان نسند الفعاليات اللي هو انشأها لل List حقته واللي هيا ownEvents
  ...
  // TODO: هذه عشان صفحة تفاصيل النادي لو نبغى نعرض الفعاليات الخاصه به فقط
  ...
  Future<void> getPastAndNewAndMyEvents({String? idForClubILead}) async {
    ...
    newEvents.clear();
    pastEvents.clear();
    namesForEventsICreated.clear(); // هذه بنستخدمها في صفحة انشاء مهمة تقريبا
    if (idForClubILead != null)
      emit(
          EventsClassifiedLoadingState()); // TODO: لأن راح نستخدمها فقط في صفحة اداره الفعاليات عشان نسوي CircleProgressIndicator()
  ...
  void createEvent(
    ...
    if (imageUrl != null) {
      ...
      FailedToCreateEventState(
          message: "حدثت مشكلة اثناء رفع الصورة يرجى المحاولة لاحقا");
    }
  }
  ...
  Future<void> getNamesOfEventsToUseInCreatingTask(
    ...
    if (ownEvents.isEmpty)
      await getPastAndNewAndMyEvents(
          idForClubILead: idForClubILead); // TODO: عشان راح نستخدم ownEvents
  ...
  // TODO: ف حالة ان التاسك حق فعاليه معينه وهو اختار فعاليه مثلا راح نروح ونجيب الداتا حقتها بناء علي اسم الفعاليه اللي اختاره من ال DropdownButton
  ...
  void createTask(
    ...
    // TODO: taskForPublicOrSpecificEvent => راح نجيب قيمتها من خلال اننا نشيك علي القيمه المختاره لو هي موجوده في id حقت الفعاليات اللي انشأتها ولا لا
  ...
  void joinToEvent(
    ...
    result.fold((serverFailure) {
      ...
      await layoutCubit
          .getMyData(); // TODO: لأن صار في تعديل في الداتا حقته وخصوصا في idForEventsJoined
  ...
  // TODO: Get Tasks for Members .... ( هذه ناقص نسويها )
  ...
  Future<void> getAvailableTasks({required UserEntity myData}) async {
    ...
    await getIDForTasksIAskedToAuthenticate(
        ...
        idForClubsIMemberIn: myData
            .idForClubsMemberIn); // TODO: عشان بس لو رسلت طلب اظهر للمستخدم انه تم ارسال طلب عشان مايرسل طلب ثاني
  ...
  // TODO: راح ترجع id حق التاسكات اللي انا رسلت طلب تسجيل فيها بالفعل  -- Call it after call getAvailableTasks()
